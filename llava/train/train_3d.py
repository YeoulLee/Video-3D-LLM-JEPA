...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        # JEPA-only setup: drop training samples whose scene has no pre-extracted JEPA file.
        if getattr(data_args, "jepa_feature_folder", None):
            kept = []
            for sample in self.list_data_dict:
                if "video" not in sample:
                    kept.append(sample)
                    continue
                scene_id = sample["video"].split("/")[-1]
                jepa_path = os.path.join(data_args.jepa_feature_folder, scene_id) + ".pt"
                if os.path.exists(jepa_path):
                    kept.append(sample)
            removed = len(self.list_data_dict) - len(kept)
            self.list_data_dict = kept
            rank0_print(f"[JEPA] filtered {removed} samples without features; {len(self.list_data_dict)} remain.")
            if not self.list_data_dict:
                raise ValueError(f"No samples remain after JEPA filtering in {data_args.jepa_feature_folder}")
