...
class JEPAProjector(nn.Module):
    """Maps a (3D-JEPA point feature, 3D coord) pair into LLM embedding space.

    Used when the model runs in JEPA-only mode (no SigLIP vision tower). Each
    pre-extracted 3D-JEPA point becomes one visual token to the LLM.

    Kept intentionally simple (two-layer MLP) to avoid fp16/bf16 fast-path
    issues we have seen with TransformerEncoderLayer / SDPA at inference.
    Spatial 3D position is injected via the model's existing
    ``world_position_embedding`` (sin3d), so no separate coord projector is
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        object_boxes = None
        object_features = None   # ensure defined for the JEPA-only path / use_object_proposals=False
        ...
        if getattr(self.config, "use_jepa_only", False):
            # JEPA-only path: skip SigLIP entirely. The pre-extracted 3D-JEPA features
            # in video_dict become the visual tokens directly.
            ...
            if "jepa_features" not in video_dict or "jepa_coords" not in video_dict:
                raise ValueError("use_jepa_only=True but video_dict is missing 'jepa_features'/'jepa_coords'")

            projector = self.get_model().jepa_projector
